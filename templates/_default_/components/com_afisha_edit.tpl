...
<form action="{$action}" method="post" enctype="multipart/form-data">
	<table cellpadding="5">
		<tr>
            <td height="30"><span>{$LANG.CAT_AFISHA}:</span></td>
            <td>
		...
		<tr>
			...
			<td height="35">
				...
				<input name="title" type="text" id="title" class="text-input" style="width:400px" maxlength="250"  value="{$item.title|escape:'html'}"/>
			</td>
		</tr>
		<tr id="from_search">
			<td></td>
...
<script type="text/javascript">
	function getRubric(){
		...
		if(category_id != 0){
			$.post("/components/afisha/ajax/get_rubric.php", {value: category_id, obtype: '{/literal}{$item.obtype}{literal}'}, function(data) {
			...
			{if $form_do == 'add'}
			{literal}
			$.post("/components/afisha/ajax/get_form.php", {value: category_id}, function(dataform) {
				if(dataform!=1){
					$('.cat_form').remove();
