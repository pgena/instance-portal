...
/* ==================================================================================================== */
	...
	public function __construct(){
		...
		$this->config      = $this->inCore->loadComponentConfig('afisha');
		$this->root_cat    = $this->inDB->get_fields('cms_afisha_cats', 'parent_id=0', '*');
		...
		$this->is_can_add_by_group   = cmsUser::isUserCan('afisha/add');
		$this->is_moderator_by_group = cmsUser::isUserCan('afisha/moderate');
...
/* ==================================================================================================== */
    ...
    public function getCommentTarget($target, $target_id) {
        ...
        switch($target){
            ...
            case 'afishaitem': $item = $this->inDB->get_fields('cms_afisha_items', "id='{$target_id}'", 'title, obtype');
                              if (!$item) { return false; }
                              $result['link']  = '/afisha/read'.$target_id.'.html';
...
/* ==================================================================================================== */
    ...
    public function getCategory($category_id = 0) {
		...
		if($category_id == $this->root_cat['id']){
	        ...
	        $category = $this->inDB->get_fields('cms_afisha_cats', "id = '{$category_id}'", '*');
		}
        ...
        if (!$category['obtypes']){
            $category['obtypes'] = $this->inDB->get_field('cms_afisha_cats', "NSLeft <= {$category['NSLeft']} AND NSRight >= {$category['NSRight']} AND obtypes <> ''", 'obtypes');
        ...
        $category = cmsCore::callEvent('GET_AFISHA_CAT', $category);
...
/* ==================================================================================================== */
    ...
    public function getSubCats($category_id){
        ...
        while($cat = $this->inDB->fetch_assoc($result)){
            if (!$cat['obtypes']){
                $cat['obtypes'] = $this->inDB->get_field('cms_afisha_cats', "NSLeft <= {$cat['NSLeft']} AND NSRight >= {$cat['NSRight']} AND obtypes <> ''", 'obtypes');
            }
        ...
        $cats = cmsCore::callEvent('GET_AFISHA_SUBCATS', $cats);
    ...
    public function getAdvertsCountFromCat($left_key, $right_key) {
		...
		$sql = "SELECT i.id
				FROM cms_afisha_items i
				INNER JOIN cms_afisha_cats cat ON cat.id = i.category_id AND cat.NSLeft >= '$left_key' AND cat.NSRight <= '$right_key'
				WHERE i.published = 1";
...
/* ==================================================================================================== */
	public function getAdverts($show_all = false, $is_users = false, $is_coments = false, $is_cats = false){
		...
		$r_join   .= $is_cats ? " INNER JOIN cms_afisha_cats cat ON cat.id = i.category_id" : '';
		...
		while ($item = $this->inDB->fetch_assoc($result)){
			...
			if($is_coments){
				$item['comments'] = cmsCore::getCommentsCount('afishaitem', $item['id']);
			}
            ...
            if (!$item['file'] || !file_exists(PATH.'/images/afisha/small/'.$item['file'])){
				$item['file'] = 'nopic.jpg';
			}
            // Права доступа
		...
		$records = cmsCore::callEvent('GET_AFISHA_RECORDS', $records);
...
/* ==================================================================================================== */
    ...
    public function getRecord($item_id) {
        ...
        $sql = "SELECT i.*,
                ...
                FROM cms_afisha_items i
				INNER JOIN cms_afisha_cats a ON a.id = i.category_id
		...
		if (!$record['file'] || !file_exists(PATH.'/images/afisha/small/'.$record['file'])){
			$record['file'] = '';
		}
        ...
        $record = cmsCore::callEvent('GET_AFISHA_RECORD', $record);
...
/* ==================================================================================================== */
    ...
    public function addRecord($item){
		...
		$item_id = $this->inDB->get_last_id('cms_afisha_items');
...
/* ==================================================================================================== */
    ...
    public function updateRecord($id, $item){
        ...
        $item = cmsCore::callEvent('UPDATE_AFISHA_RECORD', $item);

        $this->inDB->update('cms_afisha_items', $item, $id);
...
/* ==================================================================================================== */
    ...
    public function deleteRecord($item_id) {
        ...
        cmsCore::callEvent('DELETE_AFISHA_RECORD', $item_id);

        $item = $this->getRecord($item_id);
		if(!$item) { return false; }

        @unlink(PATH.'/images/afisha/'.$item['file']);
        @unlink(PATH.'/images/afisha/small/'.$item['file']);
        @unlink(PATH.'/images/afisha/medium/'.$item['file']);

        $this->inDB->delete('cms_afisha_items', " id = '$item_id'", 1);

		cmsCore::deleteComments('afishaitem', $item_id);

		cmsActions::removeObjectLog('add_afisha', $item_id);
...
/* ==================================================================================================== */
    ...
    public function deleteOldRecords() {
        ...
        if ($this->config['aftertime']){
            ...
            switch ($this->config['aftertime']){
                case 'delete':  $time_sql = "DELETE FROM cms_afisha_items WHERE DATEDIFF(NOW(), pubdate) > pubdays AND pubdays > 0"; break;
                case 'hide':    $time_sql = "UPDATE cms_afisha_items SET published = 0 WHERE DATEDIFF(NOW(), pubdate) > pubdays AND pubdays > 0"; break;
            }
            if ($time_sql){
    ...
    public function clearOldVips() {
        ...
        return $this->inDB->query("UPDATE cms_afisha_items SET is_vip=0 WHERE DATE(vipdate) <= CURRENT_DATE");

    }

    public function deleteVip($item_id) {

        return $this->inDB->query("UPDATE cms_afisha_items SET is_vip=0 WHERE id = '{$item_id}'");
...
/* ==================================================================================================== */
	...
	public function uploadPhoto($old_file = '', $cat) {
		...
		$inUploadPhoto->upload_dir    = PATH.'/images/afisha/';
...
/* ==================================================================================================== */
    ...
    public function getCatCity($cat_id = 0){
        ...
        $sql = "SELECT city FROM cms_afisha_items WHERE published = 1 AND {$cat_sql} GROUP BY city";
        $result = $this->inDB->query($sql);
...
/* ==================================================================================================== */
    ...
    public function getAfishaCities($selected='', $cat=array()){
...
/* ==================================================================================================== */
	public function getTypesLinks($cat_id, $types){
		...
		foreach($types as $id=>$type){
			$type = trim($type);
			$html .= '<a class="afisha_cats_a" href="/afisha/'.$cat_id.'/type/'.urlencode(icms_ucfirst($type)).'">'.icms_ucfirst($type).'</a>, ';
		}
		$html = rtrim($html, ', ');
		return $html;
...
/* ==================================================================================================== */
	public function orderForm($orderby, $orderto, $category){
		...
		$smarty = $this->inCore->initSmarty('components', 'com_afisha_order_form.tpl');
		...
		$smarty->assign('bcities', $this->getAfishaCities($this->city, $category));
		$smarty->assign('orderby', $orderby);
		$smarty->assign('orderto', $orderto);
		$smarty->assign('action_url', '/afisha/'.$category['id']);
		ob_start();
		$smarty->display('com_afisha_order_form.tpl');
		return ob_get_clean();
...
/* ==================================================================================================== */
	public function checkLoadedByUser24h($cat){
		...
		$u_count = $this->inDB->rows_count('cms_afisha_items', "category_id = '{$cat['id']}' {$where} AND pubdate >= DATE_SUB(NOW(), INTERVAL 1 DAY)");
...
/* ==================================================================================================== */
	...
	public function checkPublished($cat, $is_edit = false){
        ...
        if ($cat['public']==2 && $this->inCore->isUserCan('afisha/autoadd')) { return 1; }
