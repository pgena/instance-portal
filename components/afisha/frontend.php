...
function afisha(){
	...
	$pagetitle = $pagetitle ? $pagetitle : $_LANG['AFISHA'];

	$inPage->setTitle($pagetitle);
	$inPage->addPathway($pagetitle, '/afisha');
...
if ($do=='view'){
    ...
    if ($category['id'] != $model->root_cat['id']) {
        ...
        $category_path  = $inDB->getNsCategoryPath('cms_afisha_cats', $category['NSLeft'], $category['NSRight']);
		if($category_path){
			foreach($category_path as $pcat){
				$inPage->addPathway($pcat['title'], '/afisha/'.$pcat['id']);
			}
		}
	...
	$inPage->addHead('<link rel="alternate" type="application/rss+xml" title="'.$_LANG['AFISHA'].'" href="'.HOST.'/rss/afisha/'.$rss_cat_id.'/feed.rss">');
	...
	$smarty = $inCore->initSmarty('components', 'com_afisha_cats.tpl');
	...
	$smarty->display('com_afisha_cats.tpl');

	// Отдаем в шаблон объявления
    $smarty = $inCore->initSmarty('components', 'com_afisha_items.tpl');
	...
	$pagebar = ($category['id'] != $model->root_cat['id']) ? cmsPage::getPagebar($total, $model->page, $category['perpage'], '/afisha/%catid%-%page%', array('catid'=>$category['id'])) : false;
    ...
    $smarty->display('com_afisha_items.tpl');
...
if ($do=='by_user'){
    ...
    $inPage->setTitle($_LANG['AFISHA'].' - '.$user['nickname']);
	$inPage->setDescription($_LANG['AFISHA'].' - '.$user['nickname']);
	...
	// Пагинация
	$pagebar = cmsPage::getPagebar($total, $model->page, 15, '/afisha/by_user_'.$login.'/page-%page%');
	...
	$smarty = $inCore->initSmarty('components', 'com_afisha_items.tpl');
    $smarty->assign('cfg', $model->config);
	$smarty->assign('page_title', $_LANG['AFISHA'].' - '.$user['nickname']);
    $smarty->assign('root_id', $model->root_cat['id']);
    $smarty->assign('items', $items);
	$smarty->assign('cat', $category);
...
if($do=='read'){
	...
	if (!$item['published']) {
		...
		$inDB->setFlag('cms_afisha_items', $model->item_id, 'hits', $item['hits']+1);
	}
	...
	$category_path = $inDB->getNsCategoryPath('cms_afisha_cats', $item['NSLeft'], $item['NSRight']);
	if($category_path){
		foreach($category_path as $pcat){
			$inPage->addPathway($pcat['title'], '/afisha/'.$pcat['id']);
		}
	...
	$smarty = $inCore->initSmarty('components', 'com_afisha_item.tpl');
	...
	$smarty->display('com_afisha_item.tpl');
...
if ($do=='additem'){
    ...
    if ( !$inCore->inRequest('submit') ) {
        ...
        if (IS_BILLING) { cmsBilling::checkBalance('afisha', 'add_item'); }
        ...
        $smarty = $inCore->initSmarty('components', 'com_afisha_edit.tpl');
        $smarty->assign('action', "/afisha/add.html");
        $smarty->assign('form_do', 'add');
        $smarty->assign('cfg', $model->config);
		$smarty->assign('cat', $cat);
		$smarty->assign('item', $item);
		$smarty->assign('pagetitle', $_LANG['ADD_ADV']);
        $smarty->assign('cities', $model->getAfishaCities($item['city']));
        ...
        $smarty->display('com_afisha_edit.tpl');
    ...
    if ( $inCore->inRequest('submit') ) {
        ...
        if ($errors){
			...
			$inCore->redirect('/afisha/'.$model->category_id.'/add.html');
        }
        ...
        if (IS_BILLING) {
            cmsBilling::process('afisha', 'add_item');
		...
		if ($published) {
			...
			cmsActions::log('add_afisha', array(
						'object' => $obtype.' '.$title,
						'object_url' => '/afisha/read'.$item_id.'.html',
						'object_id' => $item_id,
						'target' => $cat['title'],
						'target_url' => '/afisha/'.$cat['id'],
						'target_id' => $cat['id'],
						'description' => ''
			));
			cmsCore::addSessionMessage($_LANG['ADV_IS_ADDED'], 'success');
			cmsCore::callEvent('ADD_AFISHA_DONE', array('id'=>$item_id));
			$inCore->redirect('/afisha/read'.$item_id.'.html');
		}

		if (!$published) {

			$link = '<a href="/afisha/read'.$item_id.'.html">'.$obtype.' '.$title.'</a>';
			if($inUser->id){
				$user = '<a href="'.cmsUser::getProfileURL($inUser->login).'">'.$inUser->nickname.'</a>';
			} else {
				$user = $_LANG['AFISHA_GUEST'].', ip: '.$inUser->ip;
			}
			...
			$inCore->redirect('/afisha/'.$model->category_id);
		}
...
if ($do=='edititem'){
    ...
    $inPage->addPathway($item['category'], '/afisha/'.$item['cat_id']);
    $inPage->addPathway($_LANG['EDIT_ADV']);

	if (!$item['moderator']){
		cmsCore::addSessionMessage($_LANG['YOU_HAVENT_ACCESS'], 'error');
		$inCore->redirect('/afisha/read'.$item['id'].'.html');
	}

    if (!$inCore->inRequest('submit')){

        $smarty = $inCore->initSmarty('components', 'com_afisha_edit.tpl');
        $smarty->assign('action', "/afisha/edit{$item['id']}.html");
        $smarty->assign('form_do', 'edit');
        $smarty->assign('cfg', $model->config);
        $smarty->assign('cat', $cat);
        $smarty->assign('cities', $model->getAfishaCities($item['city'], $cat));
        $smarty->assign('item', $item);
    ...
    if ($inCore->inRequest('submit')){
		...
		if ($errors){ $inCore->redirect('/afisha/edit'.$item['id'].'.html'); }
		...
		cmsActions::updateLog('add_afisha', array('object' => $obtype.' '.$title), $item['id']);
		...
		if (!$published) {
			...
			$link = '<a href="/afisha/read'.$item['id'].'.html">'.$obtype.' '.$title.'</a>';
			$user = '<a href="'.cmsUser::getProfileURL($inUser->login).'">'.$inUser->nickname.'</a>';
...
if ($do == 'publish'){
	...
	// публикуем
    $inDB->setFlag('cms_afisha_items', $model->item_id, 'published', 1);

	cmsCore::callEvent('ADD_AFISHA_DONE', $item);
 	...
 	if($item['user_id']){
		...
		cmsActions::log('add_afisha', array(
					...
					'object_url' => '/afisha/read'.$item['id'].'.html',
					'object_id' => $item['id'],
					'target' => $item['category'],
					'target_url' => '/afisha/'.$item['cat_id'],
					'target_id' => $item['cat_id'],
					'description' => ''
		));

		$link = '<a href="/afisha/read'.$item['id'].'.html">'.$item['obtype'].' '.$item['title'].'</a>';
...
if ($do == 'delete'){
	...
	if (!$item['moderator']){
		cmsCore::addSessionMessage($_LANG['YOU_HAVENT_ACCESS'], 'error');
		$inCore->redirect('/afisha/'.$item['cat_id']);
	}
