...
function search_afisha($query, $look){

        $inDB   = cmsDatabase::getInstance();
		$searchModel = cms_model_search::initModel();

		$sql = "SELECT f.*, f.title as title, a.title as cat, a.id as cat_id
				FROM cms_afisha_items f
				INNER JOIN cms_afisha_cats a ON a.id = f.category_id AND a.published = 1
		...
		if ($inDB->num_rows($result)){
			...
			while($item = $inDB->fetch_assoc($result)){
				...
				$result_array['link']        = "/afisha/read".$item['id'].".html";
				$result_array['place']       = $item['cat'];
				$result_array['placelink']   = '/afisha/'.$item['cat_id'];
