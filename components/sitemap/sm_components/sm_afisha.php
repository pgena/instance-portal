...
class afisha_map extends comMaps{
    public $title = "Доска мероприятий";
    public $link = "afisha";
    
    public function __construct() {
        $this->inDB = cmsDatabase::getInstance();
        $this->total = $this->inDB->rows_count("cms_afisha_items", "published = 1");
        $this->total = $this->total + $this->inDB->rows_count("cms_afisha_cats", "published = 1");
    }

    public function FillMapsArray($html=FALSE){
        $this->map_start();
        if (!$this->generateMap and !$html){ return FALSE; }
        $cats = $this->getCategoryTree("cms_afisha_cats", $this->host . "/afisha");
        foreach ($cats as $cat){
            if ($cat['id']==1){
                $cat['title'] = "Главная страница Объявлений";
                $cat['seolink'] = $this->host . "/afisha";
            }
            ...
            $sql = "SELECT pubdate, title, id FROM cms_afisha_items WHERE category_id = '".$cat['id']."' AND published = 1";
            ...
            while($item = $this->inDB->fetch_assoc($result)){
                ...
                if ($this->generateMap){
                    $this->set_map_url(array(
                        $this->host . "/afisha/read" . $item['id'] . ".html",
                        $item['pubdate'] >= strtotime("-1 week") ? "daily" : "weekly",
                        $item['pubdate'] >= strtotime("-1 week") ? "0.9" : "0.8",
                        date("Y-m-d", $item['pubdate'])
                    ));
                }
                if ($html){ $this->set_html_item($item['title'], $this->host . "/afisha/read" . $item['id'] . ".html"); }
            }
