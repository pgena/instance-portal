...
/******************************************************************************/
    ...
    function uploadCategoryIcon($file='') {
		...
		$inUploadPhoto->upload_dir    = PATH.'/upload/afisha/';
	...
	//LOAD CURRENT CONFIG
	$cfg = $inCore->loadComponentConfig('afisha');
    $inCore->loadModel('afisha');
    $model = new cms_model_afisha();

    define('IS_BILLING', $inCore->isComponentInstalled('billing'));
    if (IS_BILLING) { $inCore->loadClass('billing'); }

	cpAddPathway('Доска мероприятий', '?view=components&do=config&id='.$_REQUEST['id']);
	echo '<h3>Доска мероприятий</h3>';
	...
	if($opt=='saveconfig'){
		...
		$inCore->saveComponentConfig('afisha', $cfg);
	...
	if ($opt=='list_items' || $opt=='list_cats' || $opt=='config' || $opt == 'show_item' || $opt == 'hide_item'){
		...
		$toolmenu[0]['title'] = 'Новое мероприятие';
		...
		$toolmenu[2]['title'] = 'Все мероприятия';
	...
	if ($opt == 'show_item'){
		if (!isset($_REQUEST['item'])){
			if (isset($_REQUEST['item_id'])){ dbShow('cms_afisha_items', (int)$_REQUEST['item_id']);  }
			echo '1'; exit;
		} else {
			dbShowList('cms_afisha_items', $_REQUEST['item']);
			$opt = 'list_items';
		}
	}
	...
	if ($opt == 'hide_item'){
		if (!isset($_REQUEST['item'])){
			if (isset($_REQUEST['item_id'])){ dbHide('cms_afisha_items', (int)$_REQUEST['item_id']);  }
			echo '1'; exit;
		} else {
			dbHideList('cms_afisha_items', $_REQUEST['item']);
			$opt = 'list_items';
		}
	}
	...
	if ($opt == 'submit_item'){
		...
		if (!empty($_REQUEST['title'])) { $title = $inCore->request('title', 'str'); } else { $title = 'Мероприятие'; }
		...
		if (isset($_FILES['picture']['tmp_name'])){
			if (@move_uploaded_file($_FILES['picture']['tmp_name'], $uploadphoto)) {
					$uploaddir = PATH.'/images/afisha/';
					$realfile = $_FILES['picture']['name'];

					$lid = dbGetFields('cms_afisha_items', 'id>0', 'id', 'id DESC');
					$lastid = $lid['id']+1;
					...
					$sql_album = "SELECT thumb1, thumb2, thumbsqr FROM cms_afisha_cats WHERE id = $category_id";
		...
		$sql = "INSERT INTO cms_afisha_items (category_id, user_id, obtype, title , content, city, pubdate, pubdays, published, file, ip)
	...
	if ($opt == 'config') {
        ...
        $items_total = $inDB->rows_count('cms_afisha_items', 'id>0');
        $items_pub = $inDB->rows_count('cms_afisha_items', 'published=1');
        $items_unpub = $items_total - $items_pub;

		if ($items_unpub) {
			echo '<div style="margin-top:10px;color:#FF3333" ><strong>Неопубликованных мероприятий:</strong> '.$items_unpub.' - <a href="index.php?view=components&do=config&id='.(int)$_REQUEST['id'].'&opt=list_items">Показать</a></div>';
		}
		...
		?>
		<?php cpCheckWritable('/images/afisha', 'folder'); ?>
		<?php cpCheckWritable('/images/afisha/medium', 'folder'); ?>
		<?php cpCheckWritable('/images/afisha/small', 'folder'); ?>
    ...
    <div id="config_tabs" style="margin-top:12px;">
        ...
        <div id="basic">
            <table width="600" border="0" cellpadding="0" cellspacing="10" class="proptable" style="border:none">
                ...
                <tr>
                    <td valign="top">
                        <strong>Комментарии для мероприятий:</strong>
                    </td>
                    <td valign="top">
                ...
                <tr>
                    <td><strong>Количество мероприятий на главной странице компонента: </strong></td>
                    <td width="250"><input name="home_perpage" type="text" id="home_perpage" size="5" value="<?php echo @$cfg['home_perpage'];?>"/> шт</td>
                </tr>
                <tr>
                    <td><strong>Число колонок для вывода мероприятий на главной странице компонента: </strong></td>
        ...
        <div id="access">
            <table width="600" border="0" cellpadding="0" cellspacing="10" class="proptable" style="border:none">
                <tr>
                    <td width="250">
                        <strong>Добавление мероприятий: </strong><br/>
                ...
                <tr>
                    <td width="250">
                        <strong>Модерация мероприятий после редактирования: </strong><br/>
                    </td>
                    <td valign="top">
                ...
                <tr>
                    <td valign="top"><strong>Срок публикации мероприятий:</strong></td>
                    <td valign="top">
                ...
                <tr>
                    <td>
                        <strong>Просроченные мероприятия: </strong><br/>
                ...
                <tr>
                    <td>
                        <strong>Разрешать пользователям продлевать сроки показа мероприятий: </strong><br/>
                        <span class="hinttext">Работает, если выбрана опция "скрывать" для просроченных мероприятий.</span>
                    </td>
                    <td valign="top">
            ...
            <table width="600" border="0" cellpadding="0" cellspacing="10" class="proptable" style="border:none">
                <tr>
                    <td width="250" valign="top">
                        <div><strong>Типы мероприятий:</strong></div>
        ...
        <div id="vip">
            <?php if (!IS_BILLING){ ?>
                <p>
                    Для поддержки VIP-мероприятий необходим компонент &laquo;<a href="http://www.instantcms.ru/billing/about.html">Биллинг пользователей</a>&raquo;
                </p>
                <p>
                    Пользователи смогут покупать VIP-статусы для своих мероприятий.<br/>
                    VIP-мероприятия выделяются цветом, всегда выводятся в начале списка <br/>
                    и могут быть выведены на главную в отдельном модуле &laquo;VIP-Мероприятия&raquo;
                </p>
                <p>
                    Без компонента &laquo;<a href="http://www.instantcms.ru/billing/about.html">Биллинг пользователей</a>&raquo; VIP-статусы для мероприятий<br/>
                    может устанавливать только администратор, при создании или редактировании <br/>
                    любого мероприятия на сайте.
                </p>
            <?php } else { ?>
                <table width="550" border="0" cellpadding="0" cellspacing="10" class="proptable" style="border:none">
                    <tr>
                        <td><strong>Разрешить VIP-Мероприятия:</strong></td>
                        <td width="250">
...
<?php
	...
	if ($opt == 'show_cat'){
		if(isset($_REQUEST['item_id'])) {
			$id = (int)$_REQUEST['item_id'];
			$sql = "UPDATE cms_afisha_cats SET published = 1 WHERE id = $id";
			dbQuery($sql) ;
			echo '1'; exit;
		}
	}
	...
	if ($opt == 'hide_cat'){
		if(isset($_REQUEST['item_id'])) {
			$id = (int)$_REQUEST['item_id'];
			$sql = "UPDATE cms_afisha_cats SET published = 0 WHERE id = $id";
			dbQuery($sql) ;
			echo '1'; exit;
		}
	}
	...
	if ($opt == 'submit_cat'){
		...
		$ns = $inCore->nestedSetsInit('cms_afisha_cats');
		$myid = $ns->AddNode($parent_id);

		if ($myid){
			$sql = "UPDATE cms_afisha_cats
					SET title='$title',
	...
	if($opt == 'delete_cat'){
		if(isset($_REQUEST['item_id'])) {
			$id = (int)$_REQUEST['item_id'];
			$sql = "SELECT id, file FROM cms_afisha_items WHERE category_id = $id";
			$result = dbQuery($sql);
			...
			$f_icon = $inDB->get_field('cms_afisha_cats', "id = '$id'", 'icon');
			//DELETE ALBUM
			dbDeleteNS('cms_afisha_cats', $id);
			if(file_exists(PATH.'/upload/afisha/cat_icons/'.$f_icon)){
				@chmod(PATH.'/upload/afisha/cat_icons/'.$f_icon, 0777);
				@unlink(PATH.'/upload/afisha/cat_icons/'.$f_icon);
			}
		}
		header('location:?view=components&do=config&id='.(int)$_REQUEST['id'].'&opt=list_cats');
	}
	...
	if ($opt == 'update_cat'){
		...
		if(isset($_REQUEST['item_id'])) {
			$id = (int)$_REQUEST['item_id'];
			 ...
			 $sql = "SELECT * FROM cms_afisha_cats WHERE id = '$id' LIMIT 1";
			 $result = dbQuery($sql) ;
			 if (mysql_num_rows($result)){
	...
	if ($opt == 'list_cats'){
		...
		$actions[1]['confirm'] = 'Вместе с рубрикой будут удалены все мероприятия. Удалить рубрику?';
		$actions[1]['link']  = '?view=components&do=config&id='.(int)$_REQUEST['id'].'&opt=delete_cat&item_id=%id%';

		//Print table
		cpListTable('cms_afisha_cats', $fields, $actions, 'parent_id>0', 'NSLeft');
	}

	if ($opt == 'list_items'){
		cpAddPathway('Мероприятия', '?view=components&do=config&id='.(int)$_REQUEST['id'].'&opt=list_items');
		...
		$fields[7]['prc'] = 'cpAfishaCatById';  $fields[7]['filter'] = 1;  $fields[7]['filterlist'] = cpGetList('cms_afisha_cats');
		...
		$actions[1]['title'] = 'Удалить';
		$actions[1]['icon']  = 'delete.gif';
		$actions[1]['confirm'] = 'Удалить мероприятие?';
		$actions[1]['link']  = '?view=components&do=config&id='.(int)$_REQUEST['id'].'&opt=delete_item&item_id=%id%';

		//Print table
		cpListTable('cms_afisha_items', $fields, $actions, '', 'pubdate DESC');
	}
			...
			<table width="610" border="0" cellpadding="0" cellspacing="10" class="proptable">
			  ...
			  <tr>
			    ...
			    <td valign="top"><select name="parent_id" id="parent_id" style="width:250px">
                    <?php  //FIND AFISHA ROOT
                        $rootid = $inDB->get_field('cms_afisha_cats', 'parent_id=0', 'id');
                    ?>
                    ...
                    <?php
                        if (isset($mod['parent_id'])){
                            echo $inCore->getListItemsNS('cms_afisha_cats', $mod['parent_id']);
                        } else {
                            echo $inCore->getListItemsNS('cms_afisha_cats');
                        }
					?>
                </select></td>
		      </tr>
                <tr>
                    ...
                    <td valign="middle"> <?php if (@$mod['icon']) { ?><img src="/upload/afisha/cat_icons/<?php echo @$mod['icon'];?>" border="0" /><?php } ?>
                        <input name="Filedata" type="file" style="width:215px; margin:0 0 0 5px; vertical-align:top" />
                    </td>
                </tr>
			  <tr>
				<td><strong>Привязать форму</strong><br/>
                    <span class="hinttext">Поля формы будут присутствовать в мероприятиях данной рубрики</span></td>
				<td>
...
шт</td>
			  ...
			  <tr>
			    <td><strong>Добавление мероприятий пользователями: </strong></td>
			  ...
			  <tr>
			    <td><strong>Максимум мероприятий </strong> <br />
...
шт</td>
			  ...
			  <tr>
			    <td><strong>Мероприятий на странице: </strong></td>
			    <td><input name="perpage" type="text" id="perpage" size="5" value="<?php echo @$mod['perpage'];?>"/>
		        шт</td>
		      </tr>
			  <tr>
                <td><p><strong>Фотографии в мероприятиях: </strong></p>                </td>
	...
	if ($opt == 'add_item' || $opt == 'edit_item'){
		...
		if ($opt=='add_item'){
			 echo '<h3>Добавить мероприятие</h3>';
		} else {
					if(isset($_REQUEST['multiple'])){
					 ...
					 $sql = "SELECT * FROM cms_afisha_items WHERE id = $id LIMIT 1";
					 $result = dbQuery($sql) ;
					 if (mysql_num_rows($result)){
						$mod = mysql_fetch_assoc($result);
					 }
        ...
        <input type="hidden" name="csrf_token" value="<?php echo cmsUser::getCsrfToken(); ?>" />
				<table width="600" border="0" cellspacing="5" class="proptable">
				  <tr>
					<td width="177"><strong>Заголовок мероприятия: </strong></td>
					<td width="311">
		...
		?>
                      <?php if (isset($mod['category_id'])){
								echo $inCore->getListItemsNS('cms_afisha_cats', $mod['category_id']);
						  } else {
						  		echo $inCore->getListItemsNS('cms_afisha_cats');
						  }
					?>
				...
				<table width="600" border="0" cellspacing="5" class="proptable">
				  <tr>
				  	<td>
						<strong>Полный текст мероприятия:</strong>
					</td>
				  </tr>
				  <tr>
					<?php
				...
				<table width="600" border="0" cellspacing="5" class="proptable">
				  ...
				  <?php } ?>
                  <tr>
                    <td><strong>Публиковать мероприятие?</strong></td>
